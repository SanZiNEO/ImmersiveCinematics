...
> **2026-08-09 修订注记（历史归档，原方案保留）**：用户确认最终规则与本节设计不同——
> ① 优先级**不能大于打断**：移除"高优先级强制抢占（忽略 interruptible）"分支，priority 仅用于队列内排序；
> ② 移除 `queueable` 参数（schema/ScriptMeta/ScriptParser 均不保留），不可打断脚本播放时新请求**一律排队**（容量 8，满则拒绝）；
> ③ 可打断脚本被替换时**立即切换**（无渐出）；
> ④ 客户端 `/icinematics queue` 查询命令不实施（与服务端命令根冲突，且播放中无法使用）。
> 实施以 0.3.4 pending-work.md 定稿方案为准。
