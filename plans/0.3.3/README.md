...
```
阶段 0 — 安全+Bug+死代码        阶段 1 — 核心重构           阶段 2 — 整洁度        阶段 3 — 收尾          阶段 4 — 架构统一
──────────────────────          ───────────────              ───────────────        ───────────────         ─────────────────
S1 路径遍历修复                  ② Timeline Widget 化 ★      C1 Scale 封装          ④ 子 Widget 相对坐标    ⑤ Clip/Keyframe 统一
...
② Timeline Widget 化
   ...
   ↓
⑤ Clip/Keyframe 统一 ← Schema 提供了字段定义，统一模型有了基础
                        TrackPlayer 实现保留各自行为
(多加载器架构)         ← 以上做完后 common/ 可直接迁
```
...
```
阶段 0 ── S1 · E1 · B1 · R1 · R3-R5 · A5 · R6 · C3 · F1 · R1 close（安全+Bug+死代码）
阶段 1 ── ② Timeline Widget 化 → ③ 双向 Schema → ① 多轨道渲染
            → C2 · G1 · G2 · P1· P3· P4（跟在重构后面顺手做）
阶段 2 ── C1 · D1 · D2 · C4 · E2（整洁度，低优先）
阶段 3 ── ④ 子 Widget 相对坐标（1h，Widget 化后自然联动）
阶段 4 ── ⑤ Clip/Keyframe 统一（运行时数据模型合并，零编辑器影响）
```

② 是基石。阶段 4 是运行时侧的深度优化，不需要改动编辑器代码。
