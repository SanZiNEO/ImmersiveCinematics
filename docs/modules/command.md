...
- **play — 播放脚本**
  ...
  - ✅ file 参数带 Tab 自动补全（递归扫描 `immersive_cinematics/scripts` 子文件夹，显示相对路径如 `chapter1/boss_fight`）（`CinematicCommand`）
...
- **文件查找**
  - ✅ `findScriptFile()` 在**游戏根脚本目录**（`immersive_cinematics/scripts`）内查找（递归支持子目录相对路径，自动补全 .json 后缀），并使用 normalize + startsWith 校验拒绝路径遍历（`CinematicCommand`）
